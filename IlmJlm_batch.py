...
def main(jobname, queue, l_max, bin_size, out_filename):
	...
	for (i, l) in  enumerate(multipole1):
		shell_script = ["#!/bin/tcsh",
					...
					"cd $PBS_O_WORKDIR"]
		if l != 0:									
			shell_script.append("IlmJlm -m /share/splinter/ug_hj/M101/256cutmapValue&Maxmasks.fits -O /share/splinter/ug_hj/M101/PCL/256Val&Maxmask_IlmJlm_" + str(i) + ".dat -N 256 -l " + str(l) + " -L " + str(multipole2[i]))
		else:
			shell_script.append("IlmJlm -m /share/splinter/ug_hj/M101/256cutmapValue&Maxmasks.fits -O /share/splinter/ug_hj/M101/PCL/256Val&Maxmask_IlmJlm_" + str(i) + ".dat -N 256" + " -L " + str(multipole2[i]))
