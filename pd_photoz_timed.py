...
def mapper1(catalog_dir, nside, ra_col, dec_col, out_dir):
    ...
    for cat in listdir(catalog_dir):
        if cat.endswith(".csv"):
            # read catalog
            try:
                c = pandas.read_csv(join(catalog_dir, cat), sep=',', low_memory=False, header=1, dtype={ra_col : np.float64, dec_col : np.float64})
                ra = c.loc[:, ra_col]
                dec = c.loc[:, dec_col]
            except:
            	pass
...
if __name__ == "__main__":
    ...
    ra_col = "ra"
    dec_col = "dec"
    out_dir = "/share/splinter/ug_hj/pd_photoz_timed"
