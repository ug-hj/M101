...
def mapper1(cat_dir, nside, ra_col, dec_col, out_dir):
    # create empty map
    hmap = np.zeros(hp.nside2npix(nside))
    
    for cat in listdir(cat_dir)[0:2]:
        if cat.endswith(".csv"):
            # read catalog
            ...
            out_filename = "countmap_" + cat[-8:-4] + ".fits"
...
def main(catalog_dir, nside, ra_col, dec_col, out_dir):
    ...
    assert isinstance(catalog_dir, basestring) == True, ("catalog_dir must be input as a string, and contain *only* catalog files")
    ...
    if not isdir(out_dir):
    	...
    	assert listdir(out_dir) == [], ("out_dir already exists, choose a new destination directory")
	...
	out_filename = os.path.basename(os.path.normpath(catalog_dir)) + "_finalcountmap.fits"
    hp.write_map(join(out_dir, out_filename), hmap)
...
if __name__=="__main__":
    catalog_dir = "/Users/Harry/catalogs_sdss"
    nside = 32
    ra_col = "ra"
    dec_col = "dec"
    out_dir = "/Users/Harry/maps_prac_sdss_2"
