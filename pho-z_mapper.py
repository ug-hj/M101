...
def mapper1(catalog_dir, nside, ra_col, dec_col, out_dir, sw=None, ew=None, weights=None):
    ...
    for cat in listdir(catalog_dir):
        if cat.endswith(".csv"):# and cat.startswith("with_header"):
            # read catalog
            c = pandas.read_csv(join(catalog_dir, cat), sep=',', header=0, dtype={ra_col : np.float64, dec_col : np.float64}, engine=None, usecols=[1,2,3,4])
            ra = c["ra"]
            dec = c["dec"]
            #stripecuts = c["stripe"]
...
if __name__ == "__main__":
    ...
    ra_col = "ra"
    dec_col = "dec"
    out_dir = "/share/splinter/ug_hj/M101/Photoz_wcuts"
    sw = None
    ew = None
    weights = None
